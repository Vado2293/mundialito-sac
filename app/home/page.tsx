...
export default function Home() {
  ...
  const [coins, setCoins] = useState<number | null>(null); // 👈 monedas
  ...
  useEffect(() => {
    ...
    setUsername(user.id || currentUserId); // o user.nombre si lo tuvieras
  }, [openPack, revealCard]); // 👈 aquí ya existen openPack y revealCard
  ...
  if (pack === "Bronze") {
    filteredPlayers = players.filter(p => p.rarity === "Bronze");
  }

  if (pack === "Silver") {
    filteredPlayers = players.filter(p => p.rarity === "Silver");
  }

  if (pack === "Gold") {
    filteredPlayers = players.filter(p => p.rarity === "Gold");
  }

  if (pack === "Legendario") {
    filteredPlayers = players.filter(p => p.rarity === "Legend");
  }
...
const openPackHandler = (pack: string) => {
  setSelectedPack(pack);

  const price = PACK_PRICES[pack]; // 💰 costo del sobre
  ...
  const id = currentUserId as string;   // <- aseguramos que es string
  const users: any = loadUsers();
  const user = users[id];
  ...
  if (user.coins < price) {
    alert("No tienes monedas suficientes para este sobre.");
    return;
  }
  ...
  // Guardar carta en colección del mismo user
 ...
 const saveToCollection = (player: any) => {
      ...
      {/* NAVBAR */}
       ...
       <nav className="relative z-50 flex flex-wrap gap-4 items-center justify-between mb-16 border border-white/10 bg-white/5 backdrop-blur-xl rounded-2xl px-4 md:px-8 py-4 md:py-5">
        ...
        <div className="hidden md:flex items-center gap-10 text-sm uppercase tracking-[0.2em]">
          <a href="/album" className="hover:text-yellow-300 transition">Álbum</a>
          <a href="/coleccion" className="hover:text-yellow-300 transition">Colección</a>
  ...
  {username && (
    <span className="hidden md:inline text-sm text-slate-300">{username}</span>
  )}
      <button
        onClick={() => {
        ...
        {isMobileMenuOpen && (
          ...
          >
            ...
            <div className="flex flex-col items-center gap-8 text-2xl font-black tracking-[0.2em] uppercase">
              <a href="/album" className="hover:text-yellow-300 transition" onClick={() => setIsMobileMenuOpen(false)}>Álbum</a>
              <a href="/coleccion" className="hover:text-yellow-300 transition" onClick={() => setIsMobileMenuOpen(false)}>Colección</a>
              <button onClick={() => setIsMobileMenuOpen(false)} className="hover:text-yellow-300 transition">Sobres</button>
              <a href="/admin" className="hover:text-yellow-300 transition" onClick={() => setIsMobileMenuOpen(false)}>Admin</a>
              
              <div className="w-full h-px bg-white/20 my-4"></div>

              {username && (
                <span className="text-sm text-slate-400">Jugador: {username}</span>
              )}
      ...
      <section className="flex justify-center mb-24">
        ...
        <motion.div
          ...
          <img
            src="/cards/sobre.png"
            alt="Sobre"
            className="absolute bottom-24 left-1/2 -translate-x-1/2 h-[400px] object-contain"
          />
...
<section className="mb-24">
  ...
  <h2 className="text-4xl font-black mb-10 tracking-[0.15em]">
    SOBRES
  </h2>

  <div className="grid ggrid-cols-1 xs:grid-cols-2">
    ...
    >
      ...
      <AnimatePresence>
       ...
       <button
            onClick={() => {
              // parar la música SIEMPRE
              if (currentAudio) {
                currentAudio.pause();
                currentAudio.currentTime = 0;
              }

              // Si aún está abriendo, solo forzar el REVEAL
              if (!revealCard) {
                setRevealCard(true);
                return;
              }

              // Si ya está revelada, cerrar y limpiar
              setOpenPack(false);
              setRevealCard(false);
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                 ...
                 <div className="mb-4">
                    <img
                      src="/cards/logo.png"
                      alt="logo"
                      className="h-65 w-auto mx-auto"
                    />
                  </div>

                  <h3
                    className="text-2xl font-black text-yellow-300 mb-2 tracking-[0.1em] text-center px-4 leading-tight"
                  >
                    {selectedPack.toUpperCase()} PACK
                  </h3>

                  <p className="text-slate-300 text-sm tracking-[0.25em] animate-pulse">
                    OPENING...
                  </p>
             ...
             <motion.div
              ...
              >
                ...
                <img
                    ...
                    className="absolute inset-0 m-auto max-h-[80%] max-w-[80%] object-contain drop-shadow-[0_0_30px_rgba(255,255,255,0.5)]-translate-y-4 md:-translate-y-10"
                  />
                ...
                <div className="absolute bottom-0 w-full bg-black/60 backdrop-blur-xl p-6 border-t border-white/10">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div
                      className={`${rarityStyles[randomPlayer.rarity as keyof typeof rarityStyles].badge} px-4 py-2 rounded-full text-sm font-black shadow-[0_0_20px_rgba(255,255,255,0.25)]`}
                    >
                      {randomPlayer.rarity}
                    </div>
